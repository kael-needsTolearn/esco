...
{
    public function SystemVal(Request $request)
    {
        $user  = Auth::user();
        // if ($user->usertype != 0) {
        ...
        $Logo_Display = DB::table('system_configurations')
                        ->where('Code_Name', 'Logo_Display')
                        ->value('CODE_VALUE');
        $data = [
            'rooms' => $Zoho_Code,
    ...
    public function fetchconfiglogo(){
        $user  = Auth::user();
        
        if($user){
            $Company_Id = DB::table('user_accesses')
    ...
    public function fetchconfiguser(Request $request){
        $user  = Auth::user();
        ...
        if($user){
            ...
            $User_Logo = DB::table('system_configurations')
            ->where('Code_Name', 'User_Logo')
            ->where('Company_Id',$Company_Id)
            ->value('CODE_VALUE');
            return response()->json([
                'data' => $User_Logo
    ...
    public function CompanyAccounts(){
        ...
        $Company_Profile = DB::table('company_profiles')
                            ->select('*')
                            ->get();
    ...
    public function SystemConfigurationsearch(Request $request){
